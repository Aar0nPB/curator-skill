...
- **主要支持**: Claude Code(本 skill 的所有功能都在 Claude Code 验证)
- **理论兼容**: OpenClaw(改安装路径即可,因为底层只是 markdown + JSON)
- **未支持**: Cursor / Codex CLI / Gemini CLI(整个 persona skill 生态尚未在这些 runtime 内,等生态成熟再适配)
